...
def get_unet():
    ...
    convMid = Conv2D(1024, (3, 3), kernel_initializer='he_normal', padding='same')(pool5)
    convMid = ELU()(convMid)
    convMid = BatchNormalization()(convMid)
    convMid = Conv2D(1024, (3, 3), kernel_initializer='he_normal', padding='same')(convMid)
    convMid = ELU()(convMid)
    convMid = BatchNormalization()(convMid)


    upMid = concatenate([Conv2DTranspose(512, (2, 2), strides=(2, 2), padding='same')(convMid), conv5], axis=3)
    convMid2 = Conv2D(256, (3, 3), padding='same')(upMid)
    convMid2 = ELU()(convMid2)
    #conv6 = BatchNormalization()(convMid2)
    convMid2 = Conv2D(256, (3, 3), padding='same')(convMid2)
    convMid2 = ELU()(convMid2)
    #conv6 = BatchNormalization()(convMid2)
    convMid2 = Dropout(0.2)(convMid2)

    up6 = concatenate([Conv2DTranspose(256, (2, 2), strides=(2, 2), padding='same')(convMid2), conv4], axis=3)
...
def postprocess(imgs):
    imgs_p = np.ndarray((imgs.shape[0], orig_img_rows, orig_img_cols), dtype=np.float32)
    for i in range(imgs.shape[0]):
        img = imgs[i]
        #img = (imgs[i] > 0.5).astype('uint8')
        imgs_p[i] = resize(img[:,:,0], (orig_img_rows, orig_img_cols), preserve_range=True)
...
def train_and_predict():
    ...
    print('Fitting model...')
    print('-'*30)
    model.fit(imgs_train, imgs_mask_train, batch_size=1, nb_epoch=100, verbose=1, shuffle=True,
